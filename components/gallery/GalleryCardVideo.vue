...
const props = defineProps({ title: { type: String }, created: { type: String }, albumEmbed: { type: String }, total: { type: Number }, redirect: { type: String } })
...
<template>
  <div class="card album-card">
    <iframe style="border-radius: 8px 0 0 0;" height="250" :src="albumEmbed" frameborder="0" allowfullscreen></iframe>


    <div class="card-body">
      <div class="d-flex flex-column justify-content-start align-self-stretch">
        <NuxtLink class="album-cover__title" :to="redirect" aria-label="Lihat Selengkapnya" title="Lihat Selengkapnya">
          {{ title }}
        </NuxtLink>

        <h5 class="card-subtitle album-cover__subtitle ">
          <div class="d-flex flex-wrap">
            <BootstrapIcon name="clock" class="me-1" /> {{ useFormatter(created) }}
          </div>
        </h5>
      </div>
    </div>
