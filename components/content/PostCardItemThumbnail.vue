...
<template>
  <div>
    <ul v-if="posts" class="list-group list-group-flush">
      <li v-for="post in posts.filter(post => post.categories.find(cat => cat === categoryId))
            ...
            <div class="col-xl-8 col-xxl-8 col-lg-8 col-md-7">
              <div class="card-body px-0 mx-0 px-md-2 mx-md-2">
                <div class="d-flex justify-content-between g-2 mb-3">
                  <span class="article-info-tag">{{ categoryName }}</span>
                  <span class="article-info-tag text-start text-secondary">{{ useFormatter(post.date_gmt)
                  }}</span>
                </div>
                <NuxtLink :to="`/article/${post.slug}`" :aria-label="`Baca Selengkapnya ${post.title}`"
